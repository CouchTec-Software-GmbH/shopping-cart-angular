...
@Component({
  ...
  template: `
            <form class="w-full max-w-xs" [formGroup]="signUpForm" (submit)="submitForm()">
                ...
                <div>
                    <button class="w-full px-4 py-3 bg-blue-600 hover:bg-blue-700 rounded-lg text-white">Create account</button>
                </div>
...
{{ errorMessage }}
      ...
      <a class="text-blue-600" (click)="signinemail.emit()">
            Sign in</a>
</p>
...
export class SignUpComponent implements AfterViewInit {
 ...
 @ViewChild('alertMessage') alertMessage!: ElementRef;
  ...
  async submitForm() {
    this.status = await this.authService.signUp(
