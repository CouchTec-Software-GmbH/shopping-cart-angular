...
import { createDefaultProjectData, get_basic_http_header, get_http_header, get_session_token_from_cookie } from '@utils/utils';
...
export class ProductService {
  ...
  async submitApplication(name: string, email: string, text: string): Promise<any> {
    try {
      let body = { name: name, email: email, message: text };
      const response = await firstValueFrom(this.http.post(`${this.apiUrl}contact`, body, get_basic_http_header()));
